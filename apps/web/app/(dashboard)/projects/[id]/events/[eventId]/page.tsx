        ...
        <div className="flex flex-wrap items-start justify-between gap-4 border-b border-gray-100 px-6 py-5">
          ...
          {lastLog && (
            <div className="flex items-center gap-2">
              <span
                className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-medium ${statusStyles[lastLog.status]}`}
              >
                <span
              ...
              <span
                className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium capitalize ${channelStyles[lastLog.channel]}`}
              >
                {lastLog.channel}
              </span>
            </div>
              ...
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50/50">
                    ...
                    <th className="hidden px-5 py-3 text-xs font-medium uppercase tracking-wider text-gray-500 sm:table-cell">
                      Error
                    </th>
                    <th className="hidden px-5 py-3 text-xs font-medium uppercase tracking-wider text-gray-500 md:table-cell">
                      Delivered
                    </th>
                  </tr>
                </thead>
                  ...
                  {event.logs.map((log) => (
                    ...
                    >
                      <td className="px-5 py-3">
                        <span
                          className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium capitalize ${channelStyles[log.channel]}`}
                        >
                          {log.channel}
                        </span>
                      </td>
                      <td className="px-5 py-3">
                        <span
                          className={`inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 text-xs font-medium ${statusStyles[log.status]}`}
                        >
                          <span
                            className={`h-1.5 w-1.5 rounded-full ${statusDots[log.status]}`}
                          />
                          {log.status}
                        </span>
                      </td>
                      ...
                      <td className="hidden whitespace-nowrap px-5 py-3 text-xs tabular-nums text-gray-500 md:table-cell">
